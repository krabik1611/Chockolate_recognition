...
def size(img):
    ...
    return cv.cvtColor(dst, cv.COLOR_BGR2RGB)
...
for file in list_files:
    ...
    new_filename = [mod_folder + file[:file.find('.')] + "_{}.jpg".format(count) for count in range(1, 4)]
    new_image = [size(img), affine(img), rotate(img)]
